...
def render_alert_section(data):
    ...
    if max_aqi >= 300:
        message = f"Hazardous AQI predicted in the next 3 days (peak {max_aqi:.1f}). Limit outdoor exposure."
    elif max_aqi >= 200:
        message = f"Very unhealthy AQI predicted (peak {max_aqi:.1f}). Sensitive groups should stay indoors."
    elif max_aqi >= 150:
        message = f"Unhealthy AQI predicted (peak {max_aqi:.1f})."
    elif max_aqi >= 100:
        message = f"AQI may reach levels unhealthy for sensitive groups (peak {max_aqi:.1f})."
    else:
        message = f"No unhealthy AQI level predicted over the next 3 days (peak {max_aqi:.1f}, {category})."

    st.markdown(
        f"""
        <div class="section-inner" style="padding-top:3rem;">
            <div class="section-eyebrow">Health Alert</div>
